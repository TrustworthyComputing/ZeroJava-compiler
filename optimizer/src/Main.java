...
public class Main {
    ...
    static void writeFacts(FactGeneratorVisitor dlVisitor, String path, boolean print_facts) {
        try {
            ...
            for (int k = 0 ; k < dlVisitor.variables_.size() ; k++)
                dlVisitor.variables_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/varMoves.iris");
            if (print_facts) System.out.println("\nvarMoves:");
            for (int k = 0 ; k < dlVisitor.var_moves_.size() ; k++)
                dlVisitor.var_moves_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/ConstMoves.iris");
            if (print_facts) System.out.println("\nConstMoves:");
            for (int k = 0 ; k < dlVisitor.const_moves_.size() ; k++)
                dlVisitor.const_moves_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/BinOpMoves.iris");
            if (print_facts) System.out.println("\nBinOpMoves:");
            for (int k = 0 ; k < dlVisitor.bin_op_moves_.size() ; k++)
                dlVisitor.bin_op_moves_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/Instructions.iris");
            if (print_facts) System.out.println("\nInstructions:");
            for (int k = 0 ; k < dlVisitor.instructions_.size() ; k++)
                dlVisitor.instructions_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/AnswerInstructions.iris");
            if (print_facts) System.out.println("\nAnswerInstructions:");
            for (int k = 0 ; k < dlVisitor.answers_.size() ; k++)
                dlVisitor.answers_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/VarUses.iris");
            if (print_facts) System.out.println("\nVarUses:");
            for (int k = 0 ; k < dlVisitor.var_uses_.size() ; k++)
                dlVisitor.var_uses_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/VarDefs.iris");
            if (print_facts) System.out.println("\nVarDefs:");
            for (int k = 0 ; k < dlVisitor.var_defs_.size() ; k++)
                dlVisitor.var_defs_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/Jumps.iris");
            if (print_facts) System.out.println("\nJumps:");
            for (int k = 0 ; k < dlVisitor.jumps_.size() ; k++)
                dlVisitor.jumps_.get(k).writerec(file, print_facts);
            file.close();

            file = new PrintWriter(path + "/Cjumps.iris");
            if (print_facts) System.out.println("\nCjumps:");
            for (int k = 0 ; k < dlVisitor.cjumps_.size() ; k++)
                dlVisitor.cjumps_.get(k).writerec(file, print_facts);
            file.close();
